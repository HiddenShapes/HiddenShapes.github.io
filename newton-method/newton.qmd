...
$$x_{n+1}:=g(x_{n}).$$
L'espoir est que la suite <span style="color: gold;">$(x_{n}), n \in ℕ$, converge vers $\bar{x}$ quand $n \to \infty$</span>.
...
Nous avons vu que pour une méthode de point fixe avec une fonction d'itération <span style="color: gold;">$g$ qui est  $C^1$</span>, il existe, $\forall n \in ℕ^*$, $\xi_{n}$ entre $\bar{x}$ et $x_n$ tel que,
...
une manière de garantir que $g$ est $C^1$ est de supposer que <span style="color: gold;">$f$ est $C^2$</span>, ce qui donne une deuxième condition sur $f$. Sous cette condition, nous avons donc que
...
- <span style="color: gold;">Condition (A)</span> : Il existe un voisinage (épointé) de $\bar{x}$ tel que $f'(x)\neq 0$
...
<div style="margin-top: -30px; margin-bottom: 10px;">Nous commençons par fixer les paramètres de notre implémentation, </div>

```{.python code-line-numbers="1-3|5-7|9-10|12-13|15-16"}
...
<br>
<div class="fragment">
<div  class="Theorembox" style="margin-top:-35px">
<b style='font-size: 1.2em;'>Remarque</b>
<hr style="margin: 0.3em auto;">
...
```{.python code-line-numbers="1-2|4-6|8-10|12|14-15"}
...
```
<pre><code class="fragment">Le zéro de la fonction est approximativement 1.7320508075688772
(5 itérations)</code></pre> 
