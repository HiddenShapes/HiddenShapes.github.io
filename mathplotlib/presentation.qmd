...
:::{.fragment}
La température est un champ scalaire de trois variables, car la température est mesurée en un point de l'espace 3D. Aujourd'hui, nous réservons notre attention aux champs scalaires de deux variables, en particulier à cet exemple:
...
* soit en <span style="color: gold;">2D</span> à l'aide de <span style="color: gold;">courbes de niveau</span> $(z_0 = f(x,y))$ ;
* soit en <span style="color: gold;">3D</span> sous la forme d'une <span style="color: gold;">surface</span> permettant de visualiser la valeur $z=f(x,y)$.
:::
...
Pour spécifier une courbe paramétrée, il suffit donc de spécifier les fonctions <span style="color: gold;">$x(t)$</span>, <span style="color: gold;">$y(t)$</span> et potentiellement <span style="color: gold;">$z(t)$</span> (si on travail en trois dimensions).
...
\end{cases}
$$
où $R$ et $d$ sont dans $\mathbb{R}^{*}$. Une courbe de cette forme est appelée une <span style="color: gold;">Trochoïde</span>.
